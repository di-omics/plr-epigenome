...
# 00_wgs_prep_1col_src1lysis_src3rxn_dst1_hhs_DRY.py):
# - REAL reagents. Does ONE reagent addition into ONE destination column, then STOPS.
# - DISCARD tips always; --return-tips is rejected (single-cell WGS preparation carryover is fatal).
# - Modes: lysis (operator-supplied volume) and reaction (operator-supplied volume), run as SEPARATE processes.
#   Biology order (authorized WGS preparation workflow source): add lysis -> operator seal/spin -> HHS operator-supplied duration at operator-supplied speed RT ->
#   spin/ice -> add reaction -> operator seal/spin -> HHS operator-supplied duration at operator-supplied speed -> spin/ice ->
#   operator-approved WGS thermal program. Seal/spin/incubation/thermocycler are MANUAL operator steps.
...
async def main():
    parser = argparse.ArgumentParser(description="WGS preparation WET single reagent addition (DISCARD tips): one addition into one destination column. Run lysis and reaction as separate processes with operator seal/spin/incubation between.")
    ...
    if args.return_tips:
        raise RuntimeError("Wet WGS preparation runs must DISCARD tips (single-cell carryover). --return-tips is not allowed here.")
    discard_tips = True
    ...
    try:
        ...
        if args.mode in ("lysis", "reaction"):
            if args.confirm != "RUN_WET_WGS_PREP_DISCARD":
                raise RuntimeError("Refusing wet run. Add: --confirm RUN_WET_WGS_PREP_DISCARD")
