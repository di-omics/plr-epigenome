...
# Fresh P50-only 4-column discard-tip camera run.
# Run:
#   python whole_genome_seq_v2_p50only_4col_discardtips_fresh.py --mode deck
#   python whole_genome_seq_v2_p50only_4col_discardtips_fresh.py
#
# Deck:
#   rail48 pos1 = p50 tips cols 1-4
#   rail48 pos2 = p300 tips cols 1-4
...
#
# This intentionally does NOT pick up p10 tips.
# P50 handles all source-to-work volumes with operator-supplied volume blowout.
...
class TipCursor:
    ...
    def take(self):
        if self.next_col > 12:
            raise RuntimeError(f"{self.name} tip cursor exhausted at column {self.next_col}")
        col = self.next_col
        self.next_col += 1
...
async def assign_deck(lh: LiquidHandler) -> Dict[str, object]:
    print("Assigning P50-only camera deck.")
    ...
    p50_tips = make_p50_tips("r48_p50_filter_tips")
    p300_tips = make_p300_tips("r48_p300_filter_tips")
    ...
    print("rail48 pos1=p50 tips, pos2=p300 tips")
...
async def main():
    parser = argparse.ArgumentParser(description="P50-only 4-column discard-tip camera protocol.")
