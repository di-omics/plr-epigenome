...
# - Typical layout: lysis source col 1, reaction source col 3 (8-strip caps block the adjacent
#   column), both into dest col 1. Set the actual column with --source-col.
# - All pipetting geometry (dispense height 1.5, XY -0.68/3.22, 7.0 uL blowout) is UNCHANGED from the
#   validated parent.
...
async def main():
    parser = argparse.ArgumentParser(description="PTA/WGA WET single reagent addition (DISCARD tips): one addition into one destination column. Run lysis and reaction as separate processes with operator seal/spin/incubation between.")
    parser.add_argument("--mode", choices=["deck", "lysis", "reaction"], default="deck")
    parser.add_argument("--return-tips", action="store_true", help="NOT ALLOWED for wet runs; present only so it can be explicitly rejected.")
    parser.add_argument("--confirm", default="")
    parser.add_argument("--source-col", type=int, default=1, help="Source rail35 pos1 column. Typical: lysis 1, reaction 3.")
    parser.add_argument("--dest-col", type=int, default=1)
    parser.add_argument("--tip-col", type=int, default=1, help="p10 tip rack column. Use a fresh column per addition.")
    args = parser.parse_args()

    if args.return_tips:
        raise RuntimeError("Wet PTA runs must DISCARD tips (single-cell carryover). --return-tips is not allowed here.")
    discard_tips = True
    ...
    try:
        ...
        if args.mode in ("lysis", "reaction"):
            if args.confirm != "RUN_WET_PTA_DISCARD":
                raise RuntimeError("Refusing wet run. Add: --confirm RUN_WET_PTA_DISCARD")

            step = STEPS[args.mode]
            print("")
            print(f"WET SINGLE ADDITION: {step.label} {step.volume_ul} uL")
            print(f"  source rail35 pos1 col {args.source_col} -> dest rail35 pos0 col {args.dest_col}; p10 tip col {args.tip_col}; DISCARD tips")
            print(f"  OPERATOR PREP: {step.manual_prep}")
            print(f"  AFTER THIS RUN (manual): {step.manual_stop}")
            ...
            print("")
            print("Robot addition complete. Operator now performs seal / spin / incubation per protocol.")
            print("No iSWAP is performed by this script; the plate stays on rail35 pos0 for the operator.")
            return
