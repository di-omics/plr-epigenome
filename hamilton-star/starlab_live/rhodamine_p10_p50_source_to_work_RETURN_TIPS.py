...
# -----------------------------------------------------------------------------
# WGS preparation (PTA) - p10 + p50 source-to-work RHODAMINE / LH geometry script
...
# - Source 96DW aspirate, p10 + p50:
#   height = 13.0
#   offset = Coordinate(0.35, 4.65, 0.0)
#   Note: Y is intentionally +0.10 higher than the last 4.55 refine.
...
# Final-ish p10-refined source/work geometry, copied to p50 source/work.
SOURCE_96DW_ASP_HEIGHT = [13.0] * 8
SOURCE_96DW_ASP_OFFSETS = [Coordinate(0.35, 4.65, 0.0)] * 8
...
async def assign_deck(lh: LiquidHandler) -> Dict[str, object]:
    ...
    print(f"SOURCE_96DW_ASP_HEIGHT = {SOURCE_96DW_ASP_HEIGHT}")
    print(f"SOURCE_96DW_ASP_OFFSETS = {SOURCE_96DW_ASP_OFFSETS}")
...
async def transfer_column(
    ...
    await lh.aspirate(
        source_wells,
        vols=vols,
        liquid_height=SOURCE_96DW_ASP_HEIGHT,
        offsets=SOURCE_96DW_ASP_OFFSETS,
