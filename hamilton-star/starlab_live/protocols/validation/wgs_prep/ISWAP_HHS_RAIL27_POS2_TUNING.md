# iSWAP HHS rail27 pos2 tuning (validation, whole-genome sequencing)
...
```
python protocols/validation/wgs_prep/test_iswap_plate_rail35pos0_to_rail27_variable.py \
  --mode move \
  --drop-position 2 \
...
```
python protocols/validation/wgs_prep/test_iswap_plate_rail27pos2_hhs_to_rail35pos0_return.py \
...
`protocols/validation/full_end_to_end/run_wgs_prep_hhs_iswap_then_pcr_enrichment_dry_return_tips.py`
runs the forward move, then the return move, then the PCR enrichment full built dry
choreography (`09_pcr_enrichment_full_built_end_to_end_dry.py`). It refuses to run
without `--confirm RUN_FULL_WGS_PREP_PCR_ENRICHMENT_DRY_RETURN_TIPS`.
