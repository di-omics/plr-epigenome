...
async def run_source_to_work(lh, r, dest_cols: List[int], discard_tips: bool, fresh_tips_per_reagent: bool):
    print("\n=== P50-ONLY SOURCE-TO-WORK ===")
    print("Using p50 tips for ALL source-to-work transfers, including former p10 volumes.")
    print("Using 5 uL p50 blowout to reduce hanging RhB droplets.")
    all_steps = P10_STEPS + P50_STEPS

    if not discard_tips:
        print("WARNING: this p50-only camera variant is intended for --discard-tips, but will return tips if discard_tips=False.")

    for dest_col in dest_cols:
        await lh.pick_up_tips(r["p50_cursor"].take() if discard_tips else r["p50_tips"]["A1:H1"])
        try:
            for src_col, vol, label in all_steps:
