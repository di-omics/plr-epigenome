...
# Targeted PCR library preparation - PCR1 master-mix addition, full 96WP dry demo
#
# Purpose:
...
PCR1_MM_STEP = Step(
    mode="pcr1-mm",
    label="Targeted PCR round 1 complete master mix",
...
async def assign_deck(lh: LiquidHandler) -> Dict[str, object]:
    print("Assigning Targeted PCR 96-well round 1/round 2 master-mix dry deck: WGS/Bio Validation 0 compatible layout...")
    ...
    work_plate = CellTreat_96_wellplate_350ul_Fb(name="rail35_pos0_targeted_pcr_round1_dest_96wp")
    source_96wp = CellTreat_96_wellplate_350ul_Fb(name="rail35_pos1_targeted_pcr_round1_mm_source_96wp")
    ...
    print(f"  transfer = {VOL_PCR1_MASTER_MIX} uL x8 by p10 for sample transfer")
    print("\nP50 geometry:")
...
async def transfer_master_mix_96wp(
    ...
    *,
    ...
    print("")
    print(f"=== TARGETED PCR 96WP DRY DEMO: {label} ===")
    ...
    print(f"Tip type: p10; tip column {tip_col}; discard_tips={discard_tips}")
...
async def main():
    parser = argparse.ArgumentParser(
        description="Targeted PCR round 2 p10 sample transfer: 2 uL source col1 to destination col1 using working deck geometry."
    )
    parser.add_argument(
        "--mode",
