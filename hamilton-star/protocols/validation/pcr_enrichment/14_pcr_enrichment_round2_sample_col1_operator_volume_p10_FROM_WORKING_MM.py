...
# PCR enrichment library preparation - operator-volume sample transfer
#
# Purpose:
# - Keep the existing WGS/validation deck unchanged.
# - Transfer operator-prepared samples using validated p10 motion geometry.
...
VOL_PCR1_MASTER_MIX = required_positive("pcr_enrichment.sample_transfer_ul")
...
PCR1_MM_STEP = Step(
    mode="pcr1-mm",
    label="PCR enrichment sample transfer",
    volume_ul=VOL_PCR1_MASTER_MIX,
    tip_type="p50",
    manual_prep=(
        "Prepare destination and source wells according to the operator-approved local SOP. "
        "Load operator-prepared samples in source rail35 pos1 column 1."
    ),
    manual_stop=f"Continue with the operator-approved local SOP/program: {THERMAL_PROGRAM_ID}.",
)
...
async def assign_deck(lh: LiquidHandler) -> Dict[str, object]:
    print("Assigning PCR enrichment operator-volume sample-transfer deck: WGS/validation compatible layout...")
...
async def main():
    parser = argparse.ArgumentParser(
        description="PCR enrichment round 2 operator-volume sample transfer using working liquid-handling geometry."
    )
    parser.add_argument(
        "--mode",
